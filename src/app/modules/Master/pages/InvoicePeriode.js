...
import { Container, makeStyles, Paper, Table, Button } from "@material-ui/core";
import useToast from "../../../components/toast";
import SVG from "react-inlinesvg";
import { toAbsoluteUrl } from "_metronic/_helpers";
import { SubWrap, Flex, Input } from "./style";
...
const InvoicePeriode = (props) => {
  ...
  const getInvoicePeriodsData = () => {
    ...
    getInvoicePeriods()
      .then((response) => {
        setInvoicePeriodsData(response.data.data);
        setLoadData(false);
      })
      .catch(() => {
        setLoadData(false);
              ...
              <Flex style={{ justifyContent: "center" }}>
                ...
                >
                  <div style={{ justifyContent: "center", display: "flex" }}>
                    <h3>
                      <FormattedMessage id="TITLE.MODAL_UPDATE.TITLE" />
                    </h3>
                  </div>
                  <div style={{ justifyContent: "center", display: "flex" }}>
                    <div style={{ width: "70%", alignSelf: "center" }}>
                      <Input
                        label={intl.formatMessage({
                          id:
                            "TITLE.MASTER_DATA.INVOICE_PERIODS.TABLE_HEADER.NAME",
                        })}
                        type="text"
                        variant="outlined"
                        name="name"
                      ...
                      />
                      <p
                        style={{ textAlign: "center", color: "red", margin: 5 }}
                      >
                    ...
                    <div style={{ width: "70%", alignSelf: "center" }}>
                      <Input
                        label={intl.formatMessage({
                          id:
                            "TITLE.MASTER_DATA.INVOICE_PERIODS.TABLE_HEADER.ACCEPTED_FROM_TIME",
                        })}
                        type="number"
                        variant="outlined"
                      ...
                      <p
                        style={{ textAlign: "center", color: "red", margin: 5 }}
                      >
                        {formik.touched.accepted_from_day &&
                        formik.errors.accepted_from_day
                          ? formik.errors.accepted_from_day
                          : null}
                      </p>
                    </div>
                    <div style={{ width: "70%", alignSelf: "center" }}>
                      <Input
                        label={intl.formatMessage({
                          id:
                            "TITLE.MASTER_DATA.INVOICE_PERIODS.TABLE_HEADER.ACCEPTED_THRU_TIME",
                        })}
                        type="number"
                        variant="outlined"
                      ...
                      <p
                        style={{ textAlign: "center", color: "red", margin: 5 }}
                      >
                        {formik.touched.accepted_thru_day &&
                        formik.errors.accepted_thru_day
                          ? formik.errors.accepted_thru_day
                          : null}
                      </p>
                    </div>
            ...
            <div className="table-wrapper-scroll-y my-custom-scrollbar">
                  ...
                  <Table className="table-bordered overflow-auto">
                    <thead>
                      <tr>
                        ...
                        <th className="bg-primary text-white align-middle">
                          <FormattedMessage id="TITLE.MASTER_DATA.INVOICE_PERIODS.TABLE_HEADER.NAME" />
                        </th>
                        <th className="bg-primary text-white align-middle">
                          <FormattedMessage id="TITLE.MASTER_DATA.INVOICE_PERIODS.TABLE_HEADER.ACCEPTED_FROM_TIME" />
                        </th>
                        <th className="bg-primary text-white align-middle">
                          <FormattedMessage id="TITLE.MASTER_DATA.INVOICE_PERIODS.TABLE_HEADER.ACCEPTED_THRU_TIME" />
                        </th>
                        <th className="bg-primary text-white align-middle text-center">
                      ...
                      {invoicePeriodsData.map((item, index) => {
                        return (
                          <tr key={index.toString()}>
                            ...
                            <td>{item.name}</td>
                            <td>{item.accepted_from_day}</td>
                            <td>{item.accepted_thru_day}</td>
              ...
              <div className="table-loading-data">
                <div className="text-center font-weight-bold">
                  <div
                    className={`table-loading-data-potition ${
                      errorData ? "text-danger" : null
                    }`}
                  >
                    {loadData && (
                      <span>
