...
import { getVendors, getContractByPic, assignPic } from "../_redux/UserManagementCrud";
...
function PicRoles(props) {
    ...
    const handleModal = (index) => {
        ...
        getContractByPic(picData[index].pic_id)
            .then((result) => {
    ...
    const handleClick = (e, index) => {
        ...
        getContractVendor(vendor_id, "")
            .then((result) => {
                const data = result.data.data.map(function (row) { return { value: row.contract_id, label: row.contract_no } })
            ...
            <Card>
                <CardBody>
                                        ...
                                        {data.map((item, index) => {
                                            return (
                                                <tr key={index.toString()}>
                                                    <td>{index + 1}</td>
                                                    <td>{item.code}</td>
                                                    <td><a href="#" onClick={(e) => handleClick(e, index)}>{item.full_name}</a></td>
                                                    <td>{item.state}</td>
                                                    <td style={{ maxWidth: "1px" }} className="text-truncate">
                                                        {item.pic_vendor}
                                                    </td>
                                                </tr>
                                            );
                    ...
                    <div className="table-wrapper-scroll-y my-custom-scrollbar">
                        <div className="segment-table">
                            <div className="hecto-10">
                                <table className="table-bordered overflow-auto" ref={picTableRef}>
                                    <thead>
                                        <tr>
                                            <th className="bg-primary text-white text-center align-middle td-5">
                                                <FormattedMessage id="TITLE.NO" />
