...
export const SelectStyled = (props) => {
  ...
  return (
    <div className="form-group">
      <label for="doc_type" className="h3">{label}</label>
...
export const CheckBoxStyled = (props) => {
  ...
  return (
    <div className="form-group">
      <label for="doc_type" className="h3">{label}</label>
      {list.find(item => item.id === +keyId).document.map((item) => (
        <div className="form-check">
          <input className="form-check-input" type="checkbox" value={item.id} id={`check-${item.id}`} />
          <label className="form-check-label" for={`check-${keyId}`}>
            {item.name}
          </label>
        </div>
      ))}
            ...
            <tr>
              ...
              <td>
                <div className="d-flex justify-content-between flex-row">
                  ...
                  <button className="btn btn-sm p-1 mr-2" onClick={handleModalDelete}>
